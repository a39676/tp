...
public class SymbolCheck2 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < binanceFullList.size(); i++) {
			...
			if (except.contains(tmpSymbol) || mainList.contains(tmpSymbol)) {
				continue;
			}
			if (mainList.size() < mainListMaxSize) {
				mainList.add(tmpSymbol);
			} else {
				secondList.add(tmpSymbol);
			}
		}

		System.out.println("Main list: ");
		System.out.println(mainList);
		System.out.println("First 70 of second list: ");
		System.out.println(secondList.subList(0, 70));

		System.out.println("First 60: ");
		System.out.println(binanceFullList.subList(0, 60));
	}
}
